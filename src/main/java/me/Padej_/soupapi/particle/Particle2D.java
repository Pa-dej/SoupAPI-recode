...
public class Particle2D {
    ...
    public void drawOrbParticle(MatrixStack matrices, float x, float y, Color c) {
        ...
        RenderSystem.setShaderTexture(0, TexturesManager.FIREFLY_ALT);
        RenderSystem.setShaderColor(c.getRed() / 255f, c.getGreen() / 255f, c.getBlue() / 255f, (float) (opacity / 255f));
        Render2D.renderTexture(matrices, x + size / 2f, y + size / 2f, size, size, 0, 0, 256, 256, 256, 256);
    ...
    public void updatePosition(float delta) {
        ...
        deltaY *= Math.pow(0.95, delta); // Затухание скорости, нормированное для 60 FPS
        deltaX *= Math.pow(0.95, delta);

        opacity -= 2f * delta; // Уменьшение прозрачности, нормированное для 60 FPS
        size /= Math.pow(1.01, delta); // Уменьшение размера, нормированное для 60 FPS
